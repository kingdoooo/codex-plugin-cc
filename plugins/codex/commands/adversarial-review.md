...
- The multi-turn investigation still embeds the full diff as evidence up to a byte budget (default 1MB; override with `CODEX_COMPANION_INVESTIGATION_INLINE_MAX_BYTES`). Above the budget Codex re-derives the diff itself with read-only commands, which is slower.
...
- Opening or resuming the thread happens before any turn exists, so those RPCs are bounded separately: 60s each, override with `CODEX_COMPANION_SETUP_RPC_TIMEOUT_MS`. They are bookkeeping, not model work, so a healthy one answers in well under a second. A timed-out resume falls back to a fresh thread; a timed-out thread creation fails the run.
